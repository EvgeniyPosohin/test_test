...
def super_name(mentors, courses, durations):
    courses_list = []
    z = ('title', 'mentors', 'duration')
    for i, h in zip(z, (courses, mentors, durations)):
        course_dict = {i: h}
    ...
    courses_max = []
    for id in minis:
        courses_min.append(courses_list[0]['title'][id])
    for id in maxes:
        courses_max.append(courses_list[0]['title'][id])
    return {
        'short_courses': [courses_min, min_],
        'long_courses': [courses_max, max_]
    }

print(super_name(mentors,courses, durations))
